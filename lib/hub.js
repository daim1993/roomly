...
class Hub {
  ...
  publicUser(user) {
    ...
    const status = this.activeStatus(user);
    if (status) {
      view.status = status;
    }
    return view;
  ...
  activeStatus(user) {
    ...
    if (status.expiresAt && status.expiresAt <= nowMs()) {
      return null;
    }
    return { text: status.text || '', emoji: status.emoji || '' };
  }
  ...
  handlers() {
    if (!this._handlers) {
      this._handlers = {
        ...
        // Fresh ICE config on demand: clients call this right before building
        // peer connections so short-lived TURN credentials are never stale.
        'ice': (conn, m) => this.ack(conn, m.req, true, { iceServers: this.config.iceServers() }),
        ...
        'status-set': this.opStatusSet,
        'profile-full': this.opProfileFull,
  ...
  sweepExpiredServers() {
    ...
    // Timed custom statuses expire here too.
    for (const user of Object.values(this.store.state.users)) {
      if (user.status && user.status.expiresAt && user.status.expiresAt <= now) {
        user.status = null;
        this.store.markDirty();
  ...
  opEdit(conn, m) {
    ...
    const updated = this.store.editMessage(m.channelKey, message.id, content, nowMs(), extractMentions(content));
    this.ack(conn, m.req, true);
  ...
  opProfile(conn, m) {
    ...
    // Validate everything first so a rejected field never half-applies.
    let nextName = null;
    if (m.displayName !== undefined) {
      nextName = sanitizeText(m.displayName, 40).replace(/\s+/g, ' ').trim();
    ...
    if (m.avatar !== undefined) {
      if (m.avatar === null || m.avatar === '') {
        nextAvatar = null;
      } else if (typeof m.avatar === 'string' &&
                 /^\/uploads\/[\w.-]{1,120}\.(?:png|jpe?g)$/i.test(m.avatar)) {
        nextAvatar = m.avatar;
      } else {
        this.ack(conn, m.req, false, {}, 'Avatars must be a static PNG or JPG image you uploaded.');
        return;
      }
    }
    if (m.privacy !== undefined && !['everyone', 'small', 'dms'].includes(m.privacy)) {
      this.ack(conn, m.req, false, {}, 'That privacy level is not valid.');
      return;
    }
    ...
    if (m.pronouns !== undefined) {
      user.pronouns = sanitizeText(String(m.pronouns), 32).trim();
    }
    if (m.bio !== undefined) {
      user.bio = sanitizeText(String(m.bio), 190).trim();
    }
    if (m.avatar !== undefined) {
      user.avatar = nextAvatar;
    }
    if (m.privacy !== undefined) {
      user.profilePrivacy = m.privacy;
    }
  ...
  opStatusSet(conn, m) {
    const user = this.store.getUser(conn.userId);
    const text = sanitizeText(String(m.text || ''), 64).trim();
    const emoji = sanitizeText(String(m.emoji || ''), 16).trim();
    if (!text && !emoji) {
      user.status = null;
    } else {
      const ttl = Math.min(Math.max(Number(m.ttlMs) || 0, 0), 7 * 24 * 3_600_000);
      user.status = {
        text,
        emoji,
        setAt: nowMs(),
        expiresAt: ttl > 0 ? nowMs() + ttl : null
      };
    }
    this.store.markDirty();
  ...
  canSeeProfileDetails(viewerId, target) {
    if (viewerId === target.id) {
      return true;
    }
    const privacy = target.profilePrivacy || 'everyone';
    if (privacy === 'everyone') {
      return true;
    }
    const sharesDm = Object.values(this.store.state.dms)
      .some((dm) => dm.users.includes(viewerId) && dm.users.includes(target.id));
    if (sharesDm) {
      return true;
    }
    if (privacy === 'small') {
      return Object.values(this.store.state.servers).some((server) =>
        server.members[viewerId] && server.members[target.id] &&
        Object.keys(server.members).length <= 200);
    }
    return false;
  }

  opProfileFull(conn, m) {
    const target = this.store.getUser(String(m.userId || ''));
    if (!target) {
      this.ack(conn, m.req, false, {}, 'No such user.');
      return;
    }
    const visible = this.canSeeProfileDetails(conn.userId, target);
    this.ack(conn, m.req, true, {
      user: { ...this.publicUser(target), online: this.connsByUser.has(target.id) },
      createdAt: target.createdAt || null,
      bio: visible ? (target.bio || '') : null,
      detailsHidden: !visible
    });
  }
  ...
  /** Platform-admin: terminate every live connection of a user. */
  adminDisconnectUser(userId) {
    for (const conn of Array.from(this.userConns(userId))) {
      try {
        conn.ws.close(4003, 'Account disabled');
      } catch {}
    }
  }

  /** Platform-admin: remove an account and everything it owns. */
  async adminDeleteUser(userId) {
    ...
    // Servers they own die; other memberships are simply removed.
    for (const server of Object.values(state.servers)) {
      if (server.ownerId === userId) {
        await this.destroyServer(server, 'deleted');
      } else if (server.members[userId]) {
        this.removeMember(server, userId, 'left');
      }
    }
    ...
    this.adminDisconnectUser(userId);
    if (user.username) {
      delete state.usernames[user.username.toLowerCase()];
    }
