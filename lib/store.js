...
class Store {
  ...
  editMessage(channelKey, messageId, content, ts) {
    return this.log(channelKey).append({ t: 'e', id: messageId, c: content, ts });
  }
  ...
  /** Returns up to `limit` non-deleted-or-tombstoned messages before `beforeId` (exclusive). */
